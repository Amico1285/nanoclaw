...
import { AgentOutput, runCliAgent, writeTasksSnapshot } from './cli-agent-runner.js';
import {
  getAllTasks,
  getDueTasks,
  getTaskById,
    ...
    const output = await runCliAgent(
      ...
      async (agentOutput: AgentOutput) => {
        if (agentOutput.result) {
          ...
          await deps.sendMessage(task.chat_jid, agentOutput.result);
        }
        if (agentOutput.status === 'error') {
          error = agentOutput.error || 'Unknown error';
        }
      },
    );
